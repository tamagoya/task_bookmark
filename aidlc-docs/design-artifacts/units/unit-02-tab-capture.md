...
- **US-3**: 仕事状態の保存（全ウィンドウのタブを保存、保存後は全タブを閉じ＋新規ウィンドウ1タブ表示）
...
1. ユーザーが保存を実行
2. Tab Capture Serviceが全ウィンドウのタブ情報を取得（`getAllWindowsTabs()`）
3. 取得したタブのID一覧を保持したうえで、タブ情報をCalendar API連携に渡す
4. 保存成功後、`closeAllCapturedTabs(tabIds)` で保存対象のタブをすべて閉じる
5. Chrome Windows Adapter で `createWindow(['about:newtab'])` を呼び、新しいタブを1つだけ開いた新規ウィンドウを表示する
