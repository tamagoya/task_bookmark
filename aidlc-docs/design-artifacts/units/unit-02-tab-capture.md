...
- タブ情報のUI表示（サイドパネル、ウィンドウグループ・選択チェックボックス付き）
- タブの順序の保持（ウィンドウ単位・同一ウィンドウ内のインデックス順）
- **ユーザー選択タブのフィルタ（2026-06-18 追加）**: 保存・閉じる対象を `selectedTabIds` で絞り込む
- 保存対象タブの一括閉じ（タブID指定）
...
## 関連User Stories
- **US-2**: 現在のタブ状態の取得と表示（全ウィンドウのタブをウィンドウグループ表示、更新ボタン）
- **US-3**: 仕事状態の保存（選択タブを保存、選択タブのみ閉じ、無視URL適用）
...
- `getAllWindowsTabEntries()`: すべてのChromeウィンドウのタブエントリを取得（tabId, windowId 付き、表示・保存用）
